...
"""
I7 组合干预实验运行器
机制：v0.8-Scheme E | 每10回合同时执行：杀死最强鹰派 + 注入100资源
规模：1000只 | 配置：混合（300鸽/300鹰/400中立）
目的：测试组合干预是否产生非线性效应（1+1≠2）
"""
import sys
import os
...
def apply_combined_intervention(state: GameState, round_num: int) -> list:
    """每INTERVAL回合同时执行：处决最强鹰派 + 注入资源"""
    events = []
    if round_num % INTERVAL == 0 and round_num > 0:
        # 1. 处决最强鹰派
        alive = state.get_alive_agents()
        hawks = [l for l in alive if l.stance == Stance.HAWK]
        if len(hawks) >= KILL_COUNT:
            hawks.sort(key=lambda l: l.resources + l.health * 0.2, reverse=True)
            for victim in hawks[:KILL_COUNT]:
                victim.health = 0
                events.append(f"💀🔪 组合干预-处决：{victim.name}(鹰派)被清除")
        
        # 2. 注入资源
        state.resource_pool += INJECT_AMOUNT
        events.append(f"🍖 组合干预-注入：+{INJECT_AMOUNT}资源到公共池")
    
    return events


def classify_events(events, intervention_events):
    round_kill = len([e for e in intervention_events if "处决" in e])
    round_inject = len([e for e in intervention_events if "注入" in e])
    round_natural = 0
    round_conversion = 0
    ...
    return round_kill, round_inject, round_natural, round_conversion


def run_single_experiment(run_id: int) -> dict:
    print(f"\n{'='*70}")
    print(f"I7-组合干预 #{run_id + 1}/{NUM_RUNS}")
    print(f"{'='*70}")
    
    random.seed(10000 + run_id)
    ...
    print(f"干预: 每{INTERVAL}回合 = 处决最强鹰派×{KILL_COUNT} + 注入{INJECT_AMOUNT}资源")
    ...
    for round_num in range(1, MAX_ROUNDS + 1):
        int_events = apply_combined_intervention(state, round_num)
        ...
        round_kill, round_inj, round_nat, round_conv = classify_events(events, int_events)
        kill_count += round_kill
        inject_count += round_inj
        if round_inj > 0:
        ...
        timeline.append({
            ...
            "kill_this_round": round_kill,
            "inject_this_round": round_inj,
        ...
        if round_num % 10 == 0 or round_num == 1:
            print(f"回合{round_num:3d}: 存活{len(alive):4d} | "
                  ...
                  f"杀{round_kill}注{round_inj}死{round_nat}自/{round_conv}转")
    ...
    print(f"统计: 处决{kill_count}次 | 注入{inject_count}次/{total_injected} | 转换{stance_conversion_count}")
...
def main():
    print("="*70)
    print("I7 组合干预（v0.8-Scheme E）")
    print(f"配置: {NUM_LOBSTERS}只 | 鸽{NUM_DOVES}/鹰{NUM_HAWKS}/中{NUM_NEUTRAL}")
    print(f"干预: 每{INTERVAL}回合 = 处决最强鹰派 + 注入{INJECT_AMOUNT}资源")
    print(f"预期非线性: 处决降低鹰派竞争 + 注入增加资源 = 可能协同提升存活率？")
    print(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*70)
    
    results = []
    for i in range(NUM_RUNS):
        results.append(run_single_experiment(i))
    ...
    inject_counts = [r["inject_count"] for r in results]
    ...
    for i, r in enumerate(results):
        print(f"  运行{i+1}: 存活{r['survival_rate']*100:.1f}% | 处决{r['kill_count']} 注入{r['inject_count']} | 转换{r['stance_conversions']} | {r['total_rounds']}回合")
    
    print(f"\n  平均存活率: {mean_rate*100:.1f}% ± {std_dev*100:.2f}%")
    print(f"  CV: {cv:.1f}%")
    print(f"  处决均值: {sum(kill_counts)/len(kill_counts):.1f}次")
    print(f"  注入均值: {sum(inject_counts)/len(inject_counts):.1f}次")
    print(f"  转换均值: {sum(conv_counts)/len(conv_counts):.1f}")
    print(f"\n  vs R3基准(27.0%): {(mean_rate*100)-27.0:+.1f}%")
    print(f"  vs I3靶向处决(21.3%): {(mean_rate*100)-21.3:+.1f}%")
    print(f"  vs I5高强度注入(21.7%): {(mean_rate*100)-21.7:+.1f}%")
    
    output_dir = os.path.join(os.path.dirname(__file__), '..', '..', '07-data', 'v0.8_schemeE_intervention', 'I7_combined_intervention')
    os.makedirs(output_dir, exist_ok=True)
    
    output_file = os.path.join(output_dir, f"I7_combined_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    with open(output_file, 'w') as f:
        json.dump({
            "experiment": "I7-CombinedIntervention",
            "version": "v0.8-E",
            "num_agents": NUM_LOBSTERS,
            "initial_config": {"dove": NUM_DOVES, "hawk": NUM_HAWKS, "neutral": NUM_NEUTRAL},
            "intervention": {"type": "combined", "interval": INTERVAL, "kill_count": KILL_COUNT, "inject_amount": INJECT_AMOUNT},
