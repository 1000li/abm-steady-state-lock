...
def resolve_turn(state: GameState, player_actions: List[dict], mechanism_params: dict = None) -> Tuple[GameState, List[str]]:
    ...
    for agent in list(agents):
        ...
        if agent.resources >= needed:
            agent.resources -= needed
            agent.remove_status("饥饿")
            agent.remove_status("濒死")
        else:
            # 自身不够，计算缺口
            shortfall = needed - agent.resources
            agent.resources = 0
            needy_agents.append((agent, shortfall))

    # 2. 公共池资源概率分配（强者优先的概率获取）
    if needy_agents and state.resource_pool > 0:
        # 随机打乱顺序（去除顺序歧视）
        random.shuffle(needy_agents)

        # 计算每个个体的实力（资源+健康度）
        total_power = 0
        agent_powers = []
        for agent, shortfall in needy_agents:
            power = agent.resources * 0.3 + agent.health * 0.7
            power = max(0.1, power)  # 保底0.1避免除零
            agent_powers.append((agent, shortfall, power))
            total_power += power

        # 概率获取资源
        for agent, shortfall, power in agent_powers:
            if state.resource_pool <= 0:
                break

            # 成功概率 = 自身实力 / (总实力 + 资源池调节因子)
            # 资源池越充足，整体成功率越高
            pool_factor = min(1.0, state.resource_pool / (len(needy_agents) * 2))
            success_prob = (power / total_power) * pool_factor * len(needy_agents)
            ...
            if random.random() < success_prob:
                ...
                agent.resources += acquire_amount
                ...
                events.append(f"{agent.name}成功获取{acquire_amount}资源（成功率{success_prob:.0%}）")
            else:
                # 获取失败
                events.append(f"{agent.name}争夺资源失败（成功率{success_prob:.0%}）")

    # 3. 还缺资源的个体扣生命值
    for agent, shortfall in [(l, s) for l, s in needy_agents if l.resources < CONSUMPTION_PER_TURN]:
        needed = CONSUMPTION_PER_TURN - agent.resources
        damage = needed * HUNGER_DAMAGE
        agent.health -= damage
        agent.add_status("饥饿")
        events.append(f"{agent.name}缺乏资源，生命-{damage}")
    ...
    sorted_agents = sorted(agents, key=lambda x: x.stance_score, reverse=True)

    # 记录鹰派成功掠夺用于立场更新
    hawk_success_events = []

    for agent in sorted_agents:
        if not agent.is_alive() or "隔离" in agent.status:
            continue

        if agent.stance == Stance.HAWK:
            event, success = hawk_action(agent, agents, events, mechanism_params)
            if event:
                events.append(event)
                if success:
                    hawk_success_events.append(agent.id)
    ...
    for agent in sorted_agents:
        ...
        if agent.stance == Stance.DOVE:
            event = dove_action(agent, state, agents)
            if event:
                events.append(event)
    ...
    if _get_param(mechanism_params, "desperate_attack_enabled", True):
        for agent in sorted_agents:
            ...
            # 鸽派且资源=0且濒死 → 概率尝试攻击
            if (agent.stance == Stance.DOVE and
                agent.resources == 0 and
                "濒死" in agent.status):
                ...
                if random.random() < desperate_prob:
                    ...
                    if event:
                        ...
                        if success:
                            ...
                            agent.stance_score = min(10, agent.stance_score + score)
                            events.append(f"💀 {agent.name}饿急攻击成功，彻底觉醒为鹰派")
                        else:
                            score = _get_param(mechanism_params, "desperate_attack_fail_score", 2)
                            agent.stance_score = min(10, agent.stance_score + score)
                            events.append(f"⚠️ {agent.name}饿急攻击失败，但记住了暴力")
    ...
    for agent in agents:
        ...
        # 资源转换为生命值（允许溢出超过100）
        if agent.resources > 0 and agent.health < MAX_HEALTH:
            # 计算可转换量（更积极的转换）
            max_heal = (MAX_HEALTH - agent.health) // HEAL_RATIO
            # 保留至少2单位资源，其余可用于治疗
            available = max(0, agent.resources - 2)
            heal_amount = min(max_heal, available)

            if heal_amount > 0:
                agent.resources -= heal_amount
                agent.health = min(MAX_HEALTH, agent.health + heal_amount * HEAL_RATIO)

                if agent.health > 100:
                    events.append(f"{agent.name}资源溢出转换，生命值突破上限达到{agent.health}")
                else:
                    events.append(f"{agent.name}消耗资源恢复了生命")
    ...
    if not skip_stance:
        for agent in agents:
            ...
            update_stance(agent, events, hawk_success_events, agents, mechanism_params)
            if agent.stance != old_stance:
                events.append(f"{agent.name}从{old_stance.value}变成了{agent.stance.value}")
    ...
    for agent in list(agents):
        if agent.health <= 0 and agent.is_alive():
            ...
            if agent.resources > 0:
                state.resource_pool += agent.resources
                events.append(f"💀 {agent.name}死亡，其{agent.resources}单位资源回归公共池")
                agent.resources = 0
            else:
                events.append(f"💀 {agent.name}死亡")
            agent.health = 0
    ...
    state.events_history.append({
        "round": state.round,
        "events": events.copy(),
        "agent_count": len(state.get_alive_agents()),
    })
...
def execute_player_action(state: GameState, action: dict) -> str:
    ...
    if action_type == "execute":
        target = find_agent_by_id(state, target_id)
...
def hawk_action(agent: Agent, all_agents: List[Agent], events: List[str], mechanism_params: dict = None) -> Tuple[str, bool]:
    """
    鹰派行为：抢夺资源
    ...
    """
    # 找目标（存活、非隔离、非自己）
    targets = [l for l in all_agents
               if l.id != agent.id and l.is_alive() and "隔离" not in l.status]
    ...
    for target in non_hawk_targets:
        success_prob, expected_gain, max_steal, counter_damage = _calc_attack_metrics(
            agent, target, False, mechanism_params
        )
    ...
    if (not target_non_hawk_first) or (best_non_hawk_gain < 0.3 and hawk_targets):
        for target in hawk_targets:
            success_prob, expected_gain, max_steal, counter_damage = _calc_attack_metrics(
                agent, target, True, mechanism_params
            )
    ...
    if candidates[0][2] < -0.5 and random.random() < 0.6:
        return f"⚠️ {agent.name}评估风险后放弃攻击", False
    ...
    if random.random() < success_prob:
        ...
        # 关系变化
        target.trust_to[agent.id] = target.trust_to.get(agent.id, 0) - 6
        agent.trust_to[target.id] = agent.trust_to.get(target.id, 0) - 3

        # 记录暴力
        agent.violence_history.append(f"掠夺了{target.name}")
        target.violence_history.append(f"被{agent.name}掠夺")

        target_type_str = "非鹰派" if target_type == "non_hawk" else "同类"

        # 触发事件用于立场更新
        events.append(f"__HAWK_SUCCESS__:{agent.id}:{target.id}")

        return f"⚔️ {agent.name}掠夺{target.name}({target_type_str})，抢夺{steal}资源，造成{damage}伤害", True
    else:
        # ===== 攻击失败 =====
        actual_counter = int(counter_damage * (0.8 + random.random() * 0.4))
        actual_counter = min(actual_counter, 10)
        agent.health -= actual_counter

        cost = min(1, agent.resources)
        agent.resources -= cost

        target.trust_to[agent.id] = target.trust_to.get(agent.id, 0) - 4
        agent.trust_to[target.id] = agent.trust_to.get(target.id, 0) - 2

        return f"🛡️ {agent.name}攻击{target.name}失败，遭反击-{actual_counter}生命，消耗{cost}资源", False


def _calc_attack_metrics(agent: Agent, target: Agent, is_hawk_target: bool, mechanism_params: dict = None) -> Tuple[float, float, int, int]:
    """计算攻击指标，返回: (成功率, 预期收益, 最大掠夺量, 反击伤害)
    mechanism_params: 可选的机制参数覆盖字典
    """
    # 计算攻击实力
    atk_power = agent.resources * 0.3 + agent.health * 0.5 + agent.stance_score * 0.3
...
def dove_action(agent: Agent, state: GameState, all_agents: List[Agent]) -> str:
    """鸽派行为：分享资源给需要帮助的同类（加入随机性）"""
    # 找需要帮助的鸽派同伴
    candidates = []
    for other in all_agents:
        if other.id == agent.id or not other.is_alive() or "隔离" in other.status:
            continue
        # 只帮鸽派或中立偏鸽的，不帮鹰派
        ...
        if "濒死" in other.status or other.health < 30 or other.resources < CONSUMPTION_PER_TURN:
            trust = agent.trust_to.get(other.id, 0)
    ...
    if from_private > 0 and agent.resources < from_private + 2:  # 至少留2单位自保
        return ""  # 自己储备不够，不帮

    # 执行分享
    state.resource_pool -= from_pool
    agent.resources -= from_private
    other.resources += share_amount
    other.health = min(100, other.health + 3)  # 分享带来一点安慰

    # 增进关系（被帮助的一方更感激）
    other.trust_to[agent.id] = other.trust_to.get(agent.id, 0) + 4
    agent.trust_to[other.id] = agent.trust_to.get(other.id, 0) + 2
    ...
    return f"💚 {agent.name}分享了资源给{other.name}{source}"


def update_stance(agent: Agent, events: List[str], hawk_success_ids: List[int], all_agents: List[Agent], mechanism_params: dict = None):
    """
    根据经历更新立场（方案E：有效机制组合）
    mechanism_params: 可选的机制参数覆盖字典
    hawk_success_ids: 本回合成功掠夺的鹰派ID列表
    all_agents: 所有龙虾列表（用于计算鹰派平均资源）
    """
    # 【方案E】初始化连续濒死计数器
    if not hasattr(agent, '_consecutive_starving'):
        agent._consecutive_starving = 0

    # 濒死经历 → 变鹰（生存本能）
    starvation_prob = _get_param(mechanism_params, "starvation_to_hawk_prob", 0.9)
    if any(f"{agent.name}缺乏资源" in e for e in events):
        agent._consecutive_starving += 1
        if random.random() < starvation_prob:
            score = _get_param(mechanism_params, "starvation_to_hawk_score", 3)
            agent.stance_score = min(10, agent.stance_score + score)
    else:
        agent._consecutive_starving = 0  # 重置计数器
    ...
    if _get_param(mechanism_params, "consecutive_starvation_enabled", True):
        ...
        if agent._consecutive_starving >= threshold:
            score = _get_param(mechanism_params, "consecutive_starvation_score", 5)
            agent.stance_score = min(10, agent.stance_score + score)
            events.append(f"💀 {agent.name}因长期饥饿而彻底绝望")
            agent._consecutive_starving = 0  # 重置避免重复触发

    # 【方案E】相对剥夺感：鸽派看到鹰派大口吃肉 → 嫉妒变鹰
    if _get_param(mechanism_params, "relative_deprivation_enabled", True):
        if agent.stance == Stance.DOVE:
            hawks = [l for l in all_agents if l.stance == Stance.HAWK and l.is_alive()]
            if hawks and agent.resources < _get_param(mechanism_params, "relative_deprivation_self_resources", 3):
                ...
                if avg_hawk_resources > dep_threshold:
                    ...
                    if random.random() < dep_prob:
                        ...
                        agent.stance_score = min(10, agent.stance_score + score)
                        events.append(f"{agent.name}眼红鹰派的资源，心生嫉妒")

    # 被掠夺 → 变鹰（仇恨/防备）
    attacked_prob = _get_param(mechanism_params, "attacked_to_hawk_prob", 0.8)
    if any(f"被{agent.name}掠夺" in e for e in events):
    ...
    if any("死亡" in e for e in events):
        if random.random() < witness_prob:
            if agent.stance == Stance.DOVE:
                ...
                agent.stance_score = max(-10, agent.stance_score + score)
            elif agent.stance == Stance.HAWK:
                score = _get_param(mechanism_params, "death_witness_hawk_score", 1)
                agent.stance_score = min(10, agent.stance_score + score)

    # 根据score更新stance分类
    if agent.stance_score <= -3:
        agent.stance = Stance.DOVE
    elif agent.stance_score >= 3:
        agent.stance = Stance.HAWK
    else:
        agent.stance = Stance.NEUTRAL


def find_agent_by_id(state: GameState, agent_id: int) -> Agent:
    """通过ID查找龙虾"""
